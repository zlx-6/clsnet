...
def get_model_complexity_info(model,
    ...
    model：nn.Module
    input_shape:tuple
    print_per_layer_state:是否打印每层的复杂度
    as_strings:是否以str的方式输出FLOPS和参数
    ...
    flops_model = add_flops_couting_methods(model)
...
def add_flops_couting_methods(net_main_module):
...
def batch_counter_hook(module,input,outout):
    batch_size = 1
    if len(input)>0:
        #对于多个输入，只得到第一个
        input = input[0]
        ...
        pass
        print('Warining! No positional inputs found for a module, aussming batch size is one')
...
def add_batch_counter_hook_function(module):
    ...
    module.__batch_counter_handle = handle
...
if __name__ == '__main__':
    ...
    net=simplenet()
    get_model_complexity_info(net,(3,16,16))
    print(params_to_string(1e9))
