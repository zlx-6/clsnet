...
from mmcls.cvcore.parallel import is_module_wrapper
from mmcls.cvcore.utils import mkdir_or_exist,is_str
from mmcls.cvcore.runner import get_dist_info,get_time_str,LogBuffer,HOOKS,Hook,get_priority,load_checkpoint
from mmcls.cvcore.registry import Registry,build_from_cfg
from mmcls.cvcore.utils import Config
...
class BaseRunner(metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if batch_processor is not None:
            if not callable(batch_processor):#是不是可调用的
                raise TypeError('batch_processor must be callable, but ...')
            warnings.warn('batch_processor is  deprecated, please implement train_step()'
                          'val_step() in the model instead')
